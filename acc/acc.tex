...
@inproceedings{padilla2010adaptive,
  title={An adaptive-covariance-rank algorithm for the unscented {K}alman filter},
...
@article{ghahremani2011online,
  title={Online state estimation of a synchronous generator using unscented {K}alman filter from phasor measurements units},
...
@article{ghahremani2011dynamic,
  title={Dynamic state estimation in power system by applying the extended {K}alman filter with unknown inputs to phasor measurements},
  author={Ghahremani, Esmaeil and Kamwa, Innocent},
  journal={IEEE Transactions on Power Systems},
  volume={26},
  number={4},
...
@article{qing2015decentralized,
  title={Decentralized unscented {K}alman filter based on a consensus algorithm for multi-area dynamic state estimation in power systems},
...
@Article{FRL1,
author = {Molina-Garcia, Angel and Bouffard, François and Kirschen, Daniel S.},
...
@Article{FRL2,
author = {Short, Joe and Infield, David and Freris, Leon L.},
...
@Article{FRL3,
author = {Zhao, Changhong and Topcu, Ufuk and Low, Steven H.},
...
@inproceedings{Demarco1996,
  ...
  author={DeMarco, Christopher L. and Sariashkar, J.V. and Alvarado, Fernando},
...
\title{\LARGE \bf
    ...
    \thanks{
        ...
        The authors are with the Bourns College of Engineering, University of California, Riverside, CA, USA.
    Emails: {\tt mike@izbicki.me}, {\tt saamini@ece.ucr.edu}, {\tt cshelton@cs.ucr.edu}, {\tt hamed@ece.ucr.edu}.
    }
}

\author{ Mike Izbicki, Sajjad Amini, Christian R. Shelton, and Hamed Mohsenian-Rad}
...
We also propose a low-rank modification to the Kalman filter that improves computational efficiency while maintaining the detection accuracy.
...
In this paper, we consider attacks against power system stability. Such attacks can be conducted in different ways, either on the generation side \cite{Demarco1996, Pasqualetti2012} or on the load side \cite{amini2015dynamic, Marnerides2014, Mohsenian-Rad2010}. Either way they essentially involve inserting positive feedback into various power system control mechanisms.

Recent work has made advances on how to \emph{detect} destabilizing attacks in power systems. 
Specifically, \cite{amini2015detecting} applies the fast Fourier transform (FFT) to system measurements. 
It detects frequencies that a destabilizing attack adds that are not present during normal operation. 
The presence of such new frequencies beyond certain pre-specified magnitude thresholds indicate the presence of a destabilizing attack.
...
\item It does not require prior knowledge of the number of buses that are compromised. That is, as in practice, we assume that the grid operator is not initially aware of how many buses are compromised. Nevertheless, our method can identify which buses are compromised.
...
This paper belongs to the family of studies that address destabilizing attacks against power systems. While the focus so far has been mostly on the definition and implementation of such attacks \cite{amini2015dynamic, Demarco1996, Mohsenian-Rad2010}, on the methods to protect the power system against such attacks \cite{Marnerides2014, Pasqualetti2012}, and occasionally on methods to detect such attacks \cite{amini2015detecting}, the focus in this paper is on the less explored topic of identifying the attack by finding the location(s) of the compromised buses.

In terms of the methodology used in this paper, the UKF has been used before for power system problems, e.g., to estimate the rotor angle and speed in synchronous generators \cite{ghahremani2011online}. However, no joint estimation is used, and the system is not under attack. A more recent analysis estimates the parameters of the motor controller and bus loads \cite{ghahremani2016local}. Again, the system is not under attack.

%\cite{ghahremani2011dynamic}
%Another EKF-UI paper by the same authors.
%\cite{ghahremani2016local}

This paper shows through numerical results that the current power system monitoring systems would require calculation of a large Jacobian matrix if one wants to apply the UKF without modification for the purpose of identifying destabilizing attacks. In this regard, the current study is related also to a thread of work, such as in \cite{qing2015decentralized, wang2012alternative}, that similarly have to deal with the computational issues that arise when applying the UKF to power systems. Again, these studies do not address the power system under attack and the details of the analysis are different from those in this paper.
...
(See \cite{Demarco1996, Pasqualetti2012} for more details.)
...
(See \cite{amini2015dynamic, Marnerides2014, Mohsenian-Rad2010} for more details.)
...
where $\uu^p$ is the constant term for the proportional controller and
\begin{equation}
A^p =
\begin{bmatrix}
0 & 0 & -(D^L)^{-1}K^{LG}
...
\end{aligned}
\end{equation}
%The attack will be destabilizing if the eigenvalue of $(A+BA^p)$ is greater than 1.
...
In particular, we assume the control inputs are zero-mean Gaussians with covariance $Q^n$, $Q^a$, and $Q^m$ respectively.
...
so we would expect this method to have low accuracy on large grids.
...
Once the matrix $K^{LG}$ is estimated,
...
This value is the total predicted attack on the $i$th bus in the power grid.
Also define
\begin{equation}
...
In this section we compare the performance of the proposed method in Section \ref{sec:method} with a baseline approach that does not apply the rank-1 assumption from Section \ref{sec:rank}.
We refer to the latter method as the \emph{standard} method.
In this section, we show that compared to the standard method, our proposed method can:
\begin{enumerate}
%(\emph{i})
\item
...
\node at (-5.5,-0.25) {\includegraphics[trim={6.5cm 1cm 5cm 1cm},clip,width=6cm,height=3.5cm]{img/409-clusterSmallWorld-20-addUniform-40-spike-gaussian-Unobserved-fullKL-ukf-Xhat}};
\node at (0,-0.25) {\includegraphics[trim={6.5cm 1cm 5cm 1cm},clip,width=6cm,height=3.5cm]{img/409-clusterSmallWorld-20-addUniform-40-spike3-gaussian-Unobserved-fullKL-ukf-Xhat}};
\node at (5.5,-0.25) {\includegraphics[trim={6.5cm 1cm 5cm 1cm},clip,width=6cm,height=3.5cm]{img/409-clusterSmallWorld-20-addUniform-40-spike5-gaussian-Unobserved-fullKL-ukf-Xhat}};

\node at (-5.5,3.6) {\includegraphics[trim={6.5cm 1cm 5cm 1cm},clip,width=6cm,height=3.5cm]{img/409-clusterSmallWorld-20-addUniform-40-spike-gaussian-Unobserved-rank1-ukf-Xhat}};
\node at (0,3.6) {\includegraphics[trim={6.5cm 1cm 5cm 1cm},clip,width=6cm,height=3.5cm]{img/409-clusterSmallWorld-20-addUniform-40-spike3-gaussian-Unobserved-rank1-ukf-Xhat}};
\node at (5.5,3.6) {\includegraphics[trim={6.5cm 1cm 5cm 1cm},clip,width=6cm,height=3.5cm]{img/409-clusterSmallWorld-20-addUniform-40-spike5-gaussian-Unobserved-rank1-ukf-Xhat}};
...
We test on this relatively small grid size because the standard method that estimates a full rank $K^{LG}$ matrix cannot scale to larger problems.
...
\begin{tikzpicture}
\small
\node at (0,-0.25) {\includegraphics[trim={6.5cm 1cm 5cm 1cm},clip,width=8cm,height=4cm]{img/406-clusterSmallWorld-20-addUniform-40-spikeNeg-gaussian-Unobserved-rank1-ukf-Xhat}};
...
\node at (-1.75,-1.9) {\color{darkgray}responsive load added};
\draw[->,darkgray] (-3.25,-1.85) -- (-3.5,-1.85);

\node at (1,-0.9) {\color{red}bus with responsive load};
\node at (-1,1.4) {\color{darkgreen}buses with normal loads};
