...
\title{\LARGE \bf
    Destabilizing Attacks in Power Systems
}
...
%\begin{align}
%\label{sys:orig}
%\x_{t+1} &= (A + A^p_t)\x_t + B(\uu_{f(t)} + \vv_t) + \epsilon_t
%\\
%\epsilon_t &\sim \normal{0}{Q+(t-f(t))CQ}
%\end{align}
...
%\fixme{This section should be extended with power grid specifics details on the structure of $A$, $A^p_t$ and $B$.}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section{Attack detection method}
Our attack detection procedure has two steps.
First we estimate the $A^p_t$ matrix at the current time step. %using the Unscented Kalman Filter.
Then we use a thresholding procedure to determine if the system is currently under attack.

We can estimate the $A^p_t$ matrix directly using dual state estimation.
In this technique, we augment the original dynamical system's state variables to also include the elements of $A^p_t$.
...
The attacker uses $\uu^m_t$ to manipulate the $A^p_t$ matrix.
The notation $\vectorize A^p_t$ refers to the column vector constructed by stacking the columns of $A^p_t$ on top of each other.

To make the model easier to analyze, we introduce two simplifying assumptions.
First, we note that all of the control inputs $\uu_t$, $\uu^p_t$ and $\uu^m_t$ are unobserved.
...
In particular, we assume $\uu_t\sim\normal{0}{Q}$, $\uu^p_t\sim\normal{0}{Q^p}$, and $\uu^m_t\sim\normal{0}{Q^m}$.
Second, we note that the typical attack involves only a few buses in the power grid.
So the matrix $A^p_t$ will be highly structured.
We assume the rank-1 structure $A^p_t=\bb_{1,t}\trans\bb_{2,t}$ where $\bb_{1,t}$ and $\bb_{2,t}$ are column vectors.
...
\end{aligned}
\end{equation}
This system is nonlinear, but it can be solved efficiently using the Unscented Kalman Filter (UKF) \cite{wan2000unscented}.
...
Define the function $f_t(i) = \sum_{j=1}^n {A^p_t}{(i,j)}$ to be the sum of the entries in the $i$th row of the $A^p_t$ matrix.
...
The rank-1 matrix $A^p_t=\bb_{1,t}\trans\bb_{2,t}$ is guaranteed to have at most one nonzero eigenvalue equal to $\trans\bb_{1,t}\bb_{2,t}$.
...
First, directly measure the control signal $\vv_t=\uu_t + A^p_t\x_t + \uu^p_t$.
Then, perform the FFT on $\vv_t$.
...
Unfortunately, the control signal $\vv_t$ is expensive to measure in practice on modern power grids.
So the FFT method cannot be used directly.

The UIO/FFT method extends the FFT method to solve this practical difficulty.
In the UIO/FFT method, we first design an Unknown Input Observer (UIO) to estimate the value of $\vv_t$ from the data; then we perform the FFT method on this estimate.
Rearranging the terms in Equation \ref{sys:orig} gives our UIO.
\begin{equation}
\vv_t \approx \vv_t + B^{-1}\epsilon =  B^{-1}(\x_{t+1} - A\x_t)
\end{equation}
This equation gives us a method for estimating $\vv_t$ given the current state $\x_t$ and the next state $\x_{t+1}$.

Now we are ready to see how our method simplifies the UIO/FFT method.
Observe that $\x_{t+1}$ depends on the unknown quantity $A^p_t$.
Therefore, standard techniques for solving the UIO problem also implicitly calculate $A^p_t$ as an intermediate step.
For example, \cite{witczak2014unknown} uses the unscented Kalman filter to solve the UIO problem in an approach reminiscent of the previous section.

Our fundamental contribution is to observe that the $A^p_t$ matrix contains all the information we need to determine whether an attack is occurring, so there is no need to perform the subsequent FFT step.
In fact, the FFT step necessarily loses information contained within $A^p_t$.
The FFT method is only able to determine whether an attack is occurring;
it cannot estimate either the sign or the magnitude of the attack.
...
For the first experiment, we initiated an attack at time 0.1 seconds on a random bus $i$ getting feedback from bus $j$ by setting ${A^p_t}^{(i,j)}$ so that the matrix $A+A^p_t$ has maximum eigenvalue 1.05.
As shown in Figure $\ref{fig:destab}$, this destabilizes the system.
Our estimation of $A^p_t$ quickly converges to the correct value.
...
We repeat the procedure above, but set the value of ${A^p_t}^{(i,j)}$ to $-10$.
