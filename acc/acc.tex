...
%We propose a novel method for identifying destabilizing attacks in smart power grids.
Modern power systems are vulnerable to destabilizing attacks.
Previous work is able to detect the presence of an attack in a power system,
but not able to identify the source of the attack.
We propose and analyze two methods for identifying the source.
The \emph{standard method} is a standard application of the Unscented Kalman Filter (UKF) and dual state estimation.
We show that the application of the UKF in this way has poor computational and statistical performance.
Specifically, we show that if the power grid has $\ell$ loads and $g$ generators,
the number of states in the filter grows as $O(\ell g)$.
To address these weaknesses, we propose the \emph{rank-1 method}.
In this method, the number of states in the filter grows as $O(\ell + g)$.
This slower growth rate gives better accuracy and scales to larger grid sizes.
\end{abstract}
...
Smart power grids are becoming more common because they improve the efficiency of power distribution.
The downside of smart grids is that they are more vulnerable to attack.
In this paper, we consider failures in smart power grids caused by destabilizing attacks.

An example destabilizing attack is shown in Figure 1.
...
\section{Connection to previous detection methods}

%In the previous section, we formulated our identification method as a dual state estimation problem.
In this section, we show how the identification methods presented above can be seen as a simplification of a previous detection method we call the UIO/FFT method, which is an extension of the FFT method \cite{amini2015dynamic,amini2015detecting}.

The Fast Fourier Transform (FFT) method works as follows.
First, directly measure the control signal $\uu_t=\uu_t^n + BA^p_t\x_t + \uu^p_t$.
...
Unfortunately, the control signal $\uu_t$ is expensive to measure in practice on modern power systems.
...
the value $\x_{t+1}$ depends on the unknown quantity $K^{LG}_t$.
Standard techniques for solving the UIO problem also implicitly calculate $K^{LG}_t$ as an intermediate step.
For example, applying the UIO presented in \cite{witczak2014unknown} to our problem is equivalent to using the standard method in Section \ref{sec:method} to estimate $\x_{t+1}$,
then applying \ref{eq:uio} to calculate $\uu_t$.

One of our contributions is to observe that the $A^p_t$ matrix contains all the information we need to determine whether an attack is occurring,
so there is no need to perform the full UIO procedure and subsequent FFT step.
...
Direct inspection of the $K^{LG}_t$ matrix calculated by our method gives us both.

\section{Simulation}
\label{sec:sim}

In this section we empirically compare the performance of the standard and rank-1 methods at identifying compromised buses.
We show that:
\begin{enumerate}
%(\emph{i})
\item
the rank-1 method has significantly lower error than the standard method;
%(\emph{ii})
\item
the rank-1 method is significantly faster than the standard method;
and
%(\emph{iii})
\item
the rank-1 method can distinguish between malicious and benign loads added to the system.
\end{enumerate}
Our experiments are both qualitative and quantitative.
...
\caption{
    ...
    Compromised buses are drawn in {\textbf{\color{red}bold red}},
    and uncompromised buses are drawn in {\color{darkgreen}thin green}.
    ...
    In the top row, we see that our rank-1 approximation of $K^{LG}$ provides relatively accurate predictions even when the number of attacks increases and the rank-1 assumption no longer holds.
...
We test on this small grid size because the standard method cannot scale to larger problems.
On this size problem, a single iteration of the rank-1 method takes about 1 second,
and a single iteration of the standard method takes about 1 minute.
On a problem with 100 generators and 100 loads,
a single iteration of the rank-1 method takes about 5 seconds,
and a single iteration of the standard method takes over an hour.

We follow the \emph{clusterSmallWorld} procedure for generating the power grid \cite{wang2010generating}.
...
The first experiment tests how the rank-1 and standard methods respond to 1, 3, and 5 compromised buses.
In each case, the attack begins at time 0.1 seconds.
The $A^p_t$ matrix is selected so that $(A+A^p_t)$ has maximum eigenvalue 1.05,
ensuring that the attack destabilizes the system.
Figure $\ref{fig:destab}$ shows the results.
The rank-1 algorithm clearly has better qualitative performance on this particular problem.

Next we see that our rank-1 method can differentiate between benign and malicious loads.
We randomly selected a load $i$ and generator $j$, then set the $i$th row and $j$th column of ${A^p_t}$ to $-10$.
...
The standard method (not shown) has difficulty with this problem.
...
We now explore the quantitative performance of our methods by measuring their performance on many power systems.
We generated to sets of power grids, one with 20 generators and 20 loads (as in the previous section), and the other with 100 generators and 100 loads.
The standard method was run only on the smaller grid
(because it is computationally infeasible to run it on the larger one),
and the rank-1 algorithm was run only on both grids.

A major strength of both methods is that they experienced no false positives.
We define a false positive to be the detection of an attack when no attack occurred
(it does not matter if the value of $\alpha_t$ is correct).
...
Notice that the rank-1 method operating on the 200 bus power system has much higher accuracy than the standard method operating on the significantly easier 40 bus system.
...
\node at (2,-1.5) {\color{red}\begin{tabular}{c}standard method\\1 compromised bus\end{tabular}};

\node at (2,0.25) {\color{blue}\begin{tabular}{c}standard method\\5 compromised buses\end{tabular}};

\node at (2,1.95) {\color{blue}\begin{tabular}{c}rank-1 method\\5 compromised buses\end{tabular}};

\node at (-1.75,3.25) {\color{red}\begin{tabular}{c}rank-1 method\\1 compromised bus\end{tabular}};
