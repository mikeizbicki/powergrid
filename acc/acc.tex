...
To identify attacks on the entire system, the method must be run separately on every bus.
...
This lets us identify the attack sooner.
\item
...
Consider a power system with $\mathcal{N} = \mathcal{G} \cup \mathcal{L}$ as the set of buses, where $\mathcal{G}$ and $\mathcal{L}$ are the sets of generator buses and load buses, respectively.
An example is shown in Figure \ref{fig:example}.
The dynamics of this system are commonly modeled using the linear state space equations
\begin{equation}
\dot\x = A\x + B\uu
\label{sys:norm}
\end{equation}
The state variables and inputs are decomposed as
\begin{align*}
\x
=
...
The vector $O^g$ is zero for generators with Automatic Generation Control (AGC) and non-zero for generators without AGC.
The system dynamics are decomposed as
...
\begin{align*}
  ...
  \end{bmatrix}.
\end{align*}
Also, $K^I$ and $K^P$ are diagonal matrices with diagonal entries equal to the integral and proportional controller coefficients of generators with AGC.
...
We model a load altering attack with the following modified system.
\begin{equation}
\dot\x = A\x + B(\uu + \uu^a)
,~~~
\uu^a =
...
\label{sys:attack1}
\end{equation}
Here, the vectors $P^{Ga}$ and $P^{La}$ (and hence $\uu^a$) represent loads added to the system by the attacker.
...
\end{aligned}
\end{equation}
The $(i,j)$th entry of matrix $K^{LG}$ contains the proportional gain for load $i$ getting feedback from generator $j$.
When no attack is present, all entries will be zero.
...
\end{aligned}
\end{equation}
%The adversary manipulates $A^p$ and $\uu^p$ to add positive feedback to the system with the goal of destabilizing it.
%Due to physical constraints, it is difficult for the attacker to manipulate $K^{LG}$ arbitrarily.
%Only a small number of entries will have non-zero values.
