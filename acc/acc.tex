...
The main tool that we use in this paper is the Unscented Kalman Filter (UKF) to perform dual state estimation to estimate anunknown attack matrix. We then identify the attack location(s) through a proper thresholding mechanism applied to the entries of the estimated attack matrix.
...
\node at (0,0) {
    \includegraphics[width=0.43\textwidth]{img/IEEE9.pdf}
};
...
\caption{
    ...
    The attack has added a load to bus 8 that gets positive feedback from frequency at bus 3.
    This causes instabilities throughout the power system.
    The inset shows divergence in generator 2's rotar angle frequency $\omega_2$.
    During normal operations when the system is stable, $\omega_2$ remains close to 0.
    The destabilizing attack causes $\omega_2$ to exceed safe operating levels unless corrective action is taken.
...
Consider a power system with $\mathcal{G}$ as the set of generator buses and $\mathcal{L}$ as the set of load buses. The set of all buses is denoted by $\mathcal{N} = \mathcal{G} \cup \mathcal{L}$. 
%
%Consider a power system with $\mathcal{N} = \mathcal{G} \cup \mathcal{L}$ as the set of buses (i.e. nodes), where $\mathcal{G}$ and $\mathcal{L}$ are the sets of generator buses and load buses, respectively.
%
...
which is a system for automatically adjusting the power output in response to changes in the load.
The entries of $P^G$ that are associated with generators with AGC capability are zero.
...
The coefficients corresponding to generators without AGC are zero.
This system incorporates the swing equations for generators, power flow equations for transmission grid, and the governor and load frequency control for generators with AGC.
...
See \cite{Pasqualetti2012, Demarco1996} for more details and examples.
...
\end{equation}
Here, $\uu^n$ denotes the control input under normal operation
and $\uu^a$ represents the control input added by the attacker.
...
We consider the case where the attacker uses a proportional controller on the demand side to dynamically determine the value of $\uu^a$ with feedback taken from frequency sensors.
Loads with this feedback, which are benign when the feedback is negative, are called \emph{frequency-responsive controllable loads}, and are widely used in practice \cite{FRL1, FRL2, FRL3}.
...
The attack will be destabilizing if the real component of the maximum eigenvalue of $(A+A^p)$ is positive.
...
Detecting the presence of a destabilizing attack is not difficult. Since the power network is an interconnected system the attack often affects several states. Therefore, detection can be done often by monitoring only a few state variables, e.g., see \cite{amini2015dynamic}.
However, identifying the attack, i.e., pinpointing which exact load bus is compromised, is a challenging task.
In this paper, we propose an identification method that works based on estimating matrix $K^{LG}$.
It determines which load buses are compromised. It can distinguish between destabilizing and benign loads.
It requires access only to synchronized measurements of the state vector $\x$,
...
The central idea in our attack identification procedure is to %has two steps:
%(\emph{i})
%First we rewrite the system dynamics of Equation %\ref{sys:attack2} in discrete time.
%This lets us 
properly reformulate and simplify the system model so 
that we can use the Unscented Kalman Filter (UKF) to perform dual state estimation to simultaneously estimate the entries of matrix $K^{LG}$ along with the system states \cite{wan2000unscented}.
Dual state estimation with the UKF is a standard technique,
and our main contribution here is the introduction of the steps that let us apply the UKF to our power system problem.
%(\emph{ii})
Then, we identify the attack location via a thresholding mechanism that is applied to the entries of the estimated matrix $K^{LG}$.
...
Recall from Equation \ref{eqAp} that in the definition of $A^p_t$,
...
\label{sys:dualized}
\end{equation}
Here, we introduced a new control input $\uu^m_t$ with error $\epsilon_m$.
It is unobserved and controlled by the attacker.
The attacker uses $\uu^m_t$ to manipulate the entries of $K^{LG}_t$, and hence $A^p_t$.
The notation $\vectorize K^{LG}_t$ refers to the vector constructed by stacking the columns of $K^{LG}_t$ on top of each other.

The model in Equation \ref{eq:aug} is \emph{nonlinear} because $K_t^{LG}$ appears both in the state variables and the definition of $A^p_t$.
...
We assume that each control input is mean-centered with covariance $Q^n$, $Q^a$, and $Q^m$,  respectively.
This assumption is justified experimentally in Section \ref{sec:sim}, where we conduct experiments with $\uu^a_t$ and $\uu^m_t$ that are highly non-normal,
...
With this argument in mind, in our procedure, we assume the rank-1 structure
\begin{equation}
K^{LG}_t=\kL_t\trans{\kG}_{t},
\end{equation}
where $\kL_{t}$ and $\kG_{t}$ are column vectors.
Under this assumption, we only have to estimate $\ell+g$ parameters instead of $\ell g$ parameters, where $\ell$ and $g$ give the size of matrix $K^{LG}_{t}$.
Fewer parameters means a lower computational burden and faster convergence.
The physical meaning of this assumption is that there is only one compromised bus in the system.
Interestingly, we show in Section \ref{sec:sim} that we are still able to identify attacks when more than one bus is compromised.
\end{enumerate}
...
Since the UKF is a standard technique, and due to the page limit, the implementation of the UKF is omitted here. We refer the interested reader to \cite{wan2000unscented} for details.
...
\end{equation}
to be the load bus that we predict to have the most compromised load and so is under the heaviest attack.
...
Given the estimated value of $A^p_t$, we can distinguish attacks from benign frequency responsive loads. %We can also estimate the magnitude of the attack.
Let $\beta$ be equal to the maximum eigenvalue of $A + A^p_t$, i.e., the system matrix under attack, minus the maximum eigenvalue of $A$, i.e., the original system matrix. 
%That is, $\beta = \lVert A - A^p_t\rVert_2 - \lVert A %\rVert_2$.
If $\beta$ is positive, then the attack has added positive feedback into the system,
and if $\beta$ is negative, then the load control mechanism is benign as it has added negative feedback. The relative size of $\beta$ can give insight on the amount of the feedback added.

In some situations, there is no need to calculate the maximum eigenvalue of $(A+A^p_t)$ to identify the attack.
For example, we can avoid this calculation when all of the eigenvalues of $A^p_t$ are nonnegative, such as when all of the entries of $A^p_t$ are nonnegative.
...
The rank-1 matrix $A^p_t=\kL_{t}\trans\kG_{t}$ is then guaranteed to have at most one nonzero eigenvalue equal to $\trans\kL_{t}\kG_{t}$.
Such value is an upper bound on the magnitude of the attack.
...
We begin with first a qualitative demonstration of these facts,
...
We test on this relatively small grid size because the standard method that requires a full rank implementation of the UKF cannot scale to larger problems.
On this size problem, a single iteration of our method takes about 1 second,
and a single iteration of the standard method takes about 1 minute. 
On a problem with 100 generators and 100 loads,
a single iteration of our method takes about 5 seconds,
and a single iteration of the standard method takes over an hour. The computation advantage of our proposed method is evident.
...
Matrix $A^p_t$ is selected such that $(A+A^p_t)$ has maximum eigenvalue 1.05,
...
To look carefully into how our proposed method can differentiate between benign and malicious loads, next we randomly selected a load $i$ and generator $j$, then set the $i$th row and $j$th column of ${A^p_t}$ to $-10$.
Recall that negative values of the $A^p_t$ matrix correspond to benign loads.
...
When no attack is underway, the largest entries of $A^p_t$ are typically less than $10^{-6}$.
