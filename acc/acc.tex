...
In this section we empirically compare the performance of the full rank and the rank-1 method at identifying the compromised buses.
We show that:
\begin{enumerate}
%(\emph{i})
\item
the rank-1 method has significantly lower error than the full rank method;
%(\emph{ii})
\item
the rank-1 method is significantly faster than the full rank rank method;
and
%(\emph{iii})
\item
the rank-1 method can distinguish between positive and negative feedback.
\end{enumerate}
We begin with a qualitative demonstration of these facts,
and conclude with a quantitative demonstration.
...
We test on this small grid size because the full rank method cannot scale to larger problems.
On this size problem, a single iteration of the rank-1 method takes about 1 second,
and a single iteration of the full rank method takes about 1 minute.
On a problem with 100 generators and 100 loads,
a single iteration of the rank-1 method takes about 5 seconds,
and a single iteration of the full rank method takes over an hour.
...
The first experiment has 6 separate scenarios that test how the rank-1 and full rank algorithms respond to 1, 3, and 5 compromised buses.
In each case, the initiate begins at time 0.1 seconds.
...
The full rank method (not shown) has difficulty with this problem.
...
The full rank algorithm was run only on the smaller grid
...
Notice that the rank-1 method operating on the 200 bus power system has much higher accuracy than the full rank algorithm operating on the significantly easier 40 bus system.
The full rank method's accuracy is little better than random guessing after two seconds.
